...
#if(N2>8)
/*static*/ uint16_t bufferWptr_uart1 = 0;
/*static*/ uint16_t bufferRptr_uart1 = 0;
#else
...
#if(N2>8)
/*static*/ uint16_t bufferWptr_uart2 = 0;
/*static*/ uint16_t bufferRptr_uart2 = 0;
#else
...
#if(N2>8)
/*static*/ uint16_t bufferWptr_uart3 = 0;
/*static*/ uint16_t bufferRptr_uart3 = 0;
#else
...
void BufferWrite_UART1(void) {
    ...
    bufferWptr_uart1++;
#if (N2!=8)
    bufferWptr_uart1 = bufferWptr_uart1 - ((bufferWptr_uart1 >> N2) << N2);
#endif
}
...
void BufferWrite_UART2(void) {
    ...
    bufferWptr_uart2++;
    //    if (bufferWptr_uart2 > BUFFERMAX - 1)
    //        bufferWptr_uart2 = bufferWptr_uart2 - BUFFERMAX;
#if (N2!=8)
    bufferWptr_uart2 = bufferWptr_uart2 - ((bufferWptr_uart2 >> N2) << N2); //bufferWptr_uart2%BUFFERMAX
#endif
}
...
bool BufferRead_UART1(uint8_t *data) {
    if (bufferRptr_uart1 == bufferWptr_uart1) {
        if (U1STAbits.OERR)
            U1STAbits.OERR = 0; //写数据满导致FIFO溢出，而没有清除，接受数据没有
        return 0;
    }
    *data = Buffer_uart1[bufferRptr_uart1];
    bufferRptr_uart1++;
#if (N2!=8)
    bufferRptr_uart1 = bufferRptr_uart1 - ((bufferRptr_uart1 >> N2) << N2); //bufferRptr_uart1 % BUFFERMAX; 
#endif
    return 1;
}
...
bool BufferRead_UART2(uint8_t *data) {
    ...
    *data = Buffer_uart2[bufferRptr_uart2];
    bufferRptr_uart2++;
#if (N2!=8)
    bufferRptr_uart2 = bufferRptr_uart2 - ((bufferRptr_uart2 >> N2) << N2);
#endif
...
//    for (size = 0; size < BUFFERMAX - 1; size++)
//        Buffer_uart1[size] = 0xff;
//}

//void ClrUART1Buf(UARTn f_uart) {
//    uint8_t size;
//    bufferWptr_uart1 = bufferRptr_uart1 = 0;
//    for (size = 0; size < BUFFERMAX - 1; size++)
//        Buffer_uart1[size] = 0xff;
...
void ClrUART1Buf(void) {
    uint8_t tmp;
    while (BufferRead_UART1(&tmp));
}

void ClrUART2Buf(void) {
    uint8_t tmp;
