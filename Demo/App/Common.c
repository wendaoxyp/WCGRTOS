...
uint8_t Str_Find_Head(uint8_t* str1, const uint8_t *str2, const uint8_t str1_bytes, const uint8_t str2_bytes) {
    ...
    while (cnt < str1_bytes) {
        ...
        if (*ptr == *str2) {//如果数据==head的话 
            is_find = true; //设置为找到，比如就str2就一个字符判断
