...
xComPortHandle xSerialPortInitMinimal(unsigned long ulWantedBaud, unsigned portBASE_TYPE uxQueueLength) {
    ...
    /* Setup the UART. */
    U3MODEbits.BRGH = serLOW_SPEED;
    U3MODEbits.STSEL = serONE_STOP_BIT;
    U3MODEbits.PDSEL = serEIGHT_DATA_BITS_NO_PARITY;
    U3MODEbits.RXINV = serNORMAL_IDLE_STATE;
    U3MODEbits.ABAUD = serAUTO_BAUD_OFF;
    U3MODEbits.LPBACK = serLOOPBACK_OFF;
    U3MODEbits.WAKE = serWAKE_UP_DISABLE;
    U3MODEbits.UEN = serNO_HARDWARE_FLOW_CONTROL;
    U3MODEbits.IREN = serNO_IRDA;
    U3MODEbits.USIDL = serCONTINUE_IN_IDLE_MODE;
    U3MODEbits.UARTEN = serUART_ENABLED;

    U3BRG = (unsigned short) (((float) configCPU_CLOCK_HZ / ((float) 16 * (float) ulWantedBaud)) - (float) 0.5);

    U3STAbits.URXISEL = serINTERRUPT_ON_SINGLE_CHAR;
    U3STAbits.UTXEN = serTX_ENABLE;
    U3STAbits.UTXINV = serNORMAL_IDLE_STATE;
    U3STAbits.UTXISEL0 = serINTERRUPT_ON_SINGLE_CHAR;
    U3STAbits.UTXISEL1 = serINTERRUPT_ON_SINGLE_CHAR;
    ...
    _U3RXIF = serCLEAR_FLAG;
    _U3TXIF = serCLEAR_FLAG;
    _U3RXIP = configKERNEL_INTERRUPT_PRIORITY;
    _U3TXIP = configKERNEL_INTERRUPT_PRIORITY;
    _U3TXIE = serINTERRUPT_ENABLE;
    _U3RXIE = serINTERRUPT_ENABLE;

    /* Clear the Rx buffer. */
    while (U3STAbits.URXDA == serSET_FLAG) {
        cChar = U3RXREG;
    }
...
void vSetUSER_Cop_CON(bool sta) {//UART3 485通讯
    _LATE6 = sta;
}
