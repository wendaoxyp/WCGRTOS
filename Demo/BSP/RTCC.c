...
void RTC_Refresh_Time(void) {
    ...
    // set RTCC initial time
    RCFGCALbits.RTCPTR = 3; // start the sequence
    RTCVAL = WCGTIME0bits.year; // YEAR
    RTCVAL = (WCGTIME0bits.month << 8) + WCGTIME1bits.day; // MONTH-1/DAY-1
    RTCVAL = WCGTIME1bits.hour; // WEEKDAY/HOURS
    RTCVAL = (WCGTIME2bits.min << 8) + WCGTIME2bits.sec; // MINUTES/SECONDS   
    // Enable RTCC, clear RTCWREN         
    RCFGCALbits.RTCEN = 1;
    RCFGCALbits.RTCWREN = 0;
...
void printrtccArg(void) {
    ...
    strcat(strings, "S1_t:");
    my_itoa(WCGCON3bits.delay, tmp);
    strcat(strings, tmp);

    strcat(strings, ";\tS1_c:");
    my_itoa(WCGCON3bits.count, tmp);
    strcat(strings, tmp);


    strcat(strings, ";\r\nS2_t:");
    my_itoa(WCGCON4bits.delay, tmp);
    strcat(strings, tmp);

    strcat(strings, ";\tS2_c:");
    my_itoa(WCGCON4bits.count, tmp);
    strcat(strings, tmp);
